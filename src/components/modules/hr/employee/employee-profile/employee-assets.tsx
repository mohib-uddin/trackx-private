...
const EmployeeAssets = ({ employeeData }: { employeeData: employeeType }) => {
  const {
    handleSubmit,
    control,
    formState: { isValid },
  ...
  const { useFetchAlLAllocatedAssets } = AssetsService();
  const { data: allocatedAssets, isLoading: isAllocatedAssetsLoading } =
    useFetchAlLAllocatedAssets(1);
  const employeeAssets = useMemo(() => {
    return allocatedAssets?.data.filter(
      (el) => Number(employeeData.id) === Number(el.user.id),
    );
  }, [allocatedAssets]);
  ...
  return (
    <main className={"w-full p-4"}>
      <div className={"mb-2 flex justify-between item-center gap-x-4"}>
        <h2 className={"text-xl font-[700]"}>Employee Assets:</h2>
        {/*<BaseFormModal*/}
      ...
      <div className={"flex flex-wrap w-full  gap-4"}>
        {employeeAssets?.map((el, index) => (
          <Card key={index} className="bg-[#CCE3FD] w-full max-w-[400px]">
            <CardHeader className="flex items-center justify-between">
              <p className="text-lg text-left font-bold">{el.asset.name}</p>
              {/*<BaseFormModal*/}
            ...
            <CardBody className={"w-full"}>
              <p>
                <span className={"font-bold"}>Allocated Date:</span>{" "}
                {el?.allocatedDate.split("T")[0]}
              </p>
              <p>
                <span className={"font-bold"}>Status:</span> {el?.access}
              </p>
            </CardBody>
          </Card>
        ))}
